...
export default function Wallet() {
  ...
  const pieData = [
    { name: 'Available', value: balance || 1, color: '#2563eb' },
    { name: 'Earned', value: totalEarned || 1, color: '#16a34a' },
    { name: 'Spent', value: totalSpent || 1, color: '#dc2626' },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-brand-soft border border-brand/20 rounded-card p-5">
            ...
            <Link to="/app/upload" className="btn-primary text-sm inline-flex items-center gap-2">
            ...
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: '#fff', border: '1px solid #e5e4e0', borderRadius: '8px', fontSize: '13px' }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {buyTiers.map((tier) => (
                <div key={tier.price} className={`flex items-center justify-between p-3 rounded-card border ${tier.popular ? 'border-brand bg-brand-soft' : 'border-border'}`}>
                  <div>
                    <p className="font-heading font-semibold text-ink">{tier.amount} credits</p>
                    <p className="text-sm text-ink-muted">{tier.price}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    {tier.popular && <span className="tag-blue">Best Value</span>}
                    <button className="btn-secondary text-xs px-3 py-1.5 opacity-60 cursor-not-allowed" disabled>Coming Soon</button>
                  </div>
                </div>
              ))}
            </div>
